...
/**
 * 2021-10-26
 * pagination 을 적용하기 위하여 controller method 를 변경한다
 * 전체리스트를 보여주는 list() 함수를 변경한다
 * list() 함수에 pagination 을 적용하기 위한 매개변수를 변경한다
 */
@Controller
...
//    private val logger = LoggerFactory.getLogger(BuyerController::class.java)
     ...
     * JPA 기능을 사용하여 pagination 을 구현하기 위한 하나의 도구
     *
     */
...
//        bService.insert(insertBuyer)
    ...
    /**
     * update 를 실행할때
     *  localhost:8080/buyer/update/userid 값으로
     *  URL 이 구성되어 있고 update 화면에서 저장을 누르면
     *  원래 요청했던 주소가 action 이 되어 요청되므로
     *  여기에서는 userid 필요없지만 PathVariable 로 설정해 주어야 한다
     */
    ...
    fun update(redirectAttributes: RedirectAttributes, buyer:Buyer, @PathVariable("userid") userid:String):String{
        ...
        /**
         * Spring MVC 에서는 model 에 변수를 담으면 redirect를 실행할때
         * model 에 담긴 변수를 queryString 으로 부착하여 전송을 한다
         *
        ...
        // redirectAttributes 를 사용하지 않고 작성하려면
