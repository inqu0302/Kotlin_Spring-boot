...
class HomeController {
    ...
    /**
     * lateinit var
     * 특별히 변수나 객체를 선언할때 즉시 초기화 하지 않고 나중에 초기화를
     * 하겠다 라는 의미
     *
     * Spring 환경에서는 Component(Service, Dao 등등) 객체는 코드에서
     * 직접 초기화 하지 않는다
     * 코드에서는 선언만 해두고 @Autowired 설정을 하면
     * Spring Ioc, DI 등의 기능에 의해(나중에, 필요할때)자동으로 주입된다
     *
     * 반드시 lateinit 키워드를 부착하여 선언해야 한다다     */
    @Autowired
...
//        model["USERS"] =userList
    ...
    /**
     * @ResponseBody
     * 문자열, 객체, 배열 등등의 모든 데이터를 있는 그대로
     * 또는 JSON 형태로 변환하여 client 에 보내라
     */
    @ResponseBody
